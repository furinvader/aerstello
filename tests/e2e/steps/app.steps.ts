...
let legacyGuestRequest: {mutationId:string;expectedPriceCents?:number} | undefined;
...
When('the guest retries a legacy pending self-service addition',async()=>{const request=guestPage!.context().request;const me=await (await request.get('/api/v1/guest/me')).json() as {guest:{sessionId:string}};const catalog=await (await request.get('/api/v1/guest/catalog')).json() as {data:{id:string;name:{de:string};priceCents:number}[]};const product=catalog.data.find(item=>item.name.de==='Mineralwasser')!;legacyGuestMutationId=crypto.randomUUID();expect((await request.post('/api/v1/guest/items',{headers:csrfHeaders,data:{mutationId:legacyGuestMutationId,productId:product.id,expectedPriceCents:product.priceCents}})).status()).toBe(201);const database=new pg.Client({connectionString:process.env.DATABASE_URL});await database.connect();try{await database.query('UPDATE order_items SET guest_expected_price_cents=NULL WHERE guest_mutation_id=$1',[legacyGuestMutationId])}finally{await database.end()}await guestPage!.evaluate(({sessionId,productId,mutationId})=>localStorage.setItem('skybar-guest-pending-adds',JSON.stringify({sessionId,entries:[[productId,mutationId]]})),{sessionId:me.guest.sessionId,productId:product.id,mutationId:legacyGuestMutationId});legacyGuestRequest=undefined;guestPage!.on('request',guestRequest=>{if(guestRequest.url().endsWith('/api/v1/guest/items')&&guestRequest.method()==='POST')legacyGuestRequest=guestRequest.postDataJSON() as {mutationId:string;expectedPriceCents?:number}});await guestPage!.reload();await guestPage!.locator('.product-tile').filter({hasText:'Mineralwasser'}).click();await expect.poll(()=>legacyGuestRequest).toBeTruthy();legacyGuestItemCount=((await (await request.get('/api/v1/guest/tab')).json()) as {itemCount:number}).itemCount;expect(legacyGuestRequest?.expectedPriceCents).toBe(product.priceCents)});
Then('the legacy addition reuses its mutation with the displayed price',async()=>{expect(legacyGuestRequest).toEqual(expect.objectContaining({mutationId:legacyGuestMutationId,expectedPriceCents:expect.any(Number)}))});
...
When('the same guest item mutation is submitted concurrently',async()=>{const request=guestPage!.context().request;const catalog=await (await request.get('/api/v1/guest/catalog')).json() as {data:{id:string;name:{de:string};priceCents:number}[]};const product=catalog.data.find(item=>item.name.de==='Mineralwasser')!;const data={mutationId:crypto.randomUUID(),productId:product.id,expectedPriceCents:product.priceCents};const responses=await Promise.all([request.post('/api/v1/guest/items',{headers:csrfHeaders,data}),request.post('/api/v1/guest/items',{headers:csrfHeaders,data})]);concurrentGuestItemStatuses=responses.map(response=>response.status());concurrentGuestItemCount=((await (await request.get('/api/v1/guest/tab')).json()) as {itemCount:number}).itemCount});
