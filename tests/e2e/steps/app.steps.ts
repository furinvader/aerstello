...
When('the host selects Italian on an English-locale device',async({page})=>{expect(await page.evaluate(()=>navigator.language)).toMatch(/^en/);let lastSeenAt='';await page.route('**/api/v1/account/sessions',async route=>{const response=await route.fetch();const payload=await response.json() as {data:{lastSeenAt:string;current:boolean}[]};lastSeenAt=payload.data.find(session=>session.current)!.lastSeenAt;await route.fulfill({response,json:payload})});await page.goto('/app/account');await expect(page.getByText('Dieses Gerät')).toBeVisible();expectedItalianSessionTimestamp=await page.evaluate(value=>new Date(value).toLocaleString('it'),lastSeenAt);await page.getByLabel('Sprache').selectOption('it');await page.getByRole('button',{name:'Speichern'}).click()});
...
When('a self-service price changes after the guest catalog is displayed',async({page})=>{const guestRequest=guestPage!.context().request;const catalog=await (await guestRequest.get('/api/v1/guest/catalog')).json() as {data:{id:string;name:{de:string};priceCents:number}[]};const displayed=catalog.data.find(item=>item.name.de==='Mineralwasser')!;const request=page.context().request;const products=await (await request.get('/api/v1/products')).json() as {data:{id:string;name:{de:string;it:string;en:string};description?:{de:string;it:string;en:string};priceCents:number;categoryId:string;enabled:boolean;selfServiceOnly:boolean}[]};const product=products.data.find(item=>item.id===displayed.id)!;expect((await request.patch(`/api/v1/products/${product.id}`,{headers:csrfHeaders,data:{name:product.name,...(product.description?{description:product.description}:{}),priceCents:product.priceCents+100,categoryId:product.categoryId,enabled:product.enabled,selfServiceOnly:product.selfServiceOnly}})).status()).toBe(200);const response=await guestRequest.post('/api/v1/guest/items',{headers:csrfHeaders,data:{mutationId:crypto.randomUUID(),productId:displayed.id,expectedPriceCents:displayed.priceCents}});staleGuestPriceRejected=response.status()===409;staleGuestPriceItemCount=((await (await guestRequest.get('/api/v1/guest/tab')).json()) as {itemCount:number}).itemCount});
