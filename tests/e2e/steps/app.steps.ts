...
When('administrators submit conflicting room orders concurrently',async({page})=>{const request=page.context().request;const rooms=await (await request.get('/api/v1/rooms')).json() as {data:{id:string}[]};const [a,b,c]=rooms.data.map(room=>room.id);concurrentRoomOrderStatuses=[];for(let attempt=0;attempt<8;attempt+=1){const responses=await Promise.all([request.put('/api/v1/rooms/order',{headers:csrfHeaders,data:{ids:[b,a,c]}}),request.put('/api/v1/rooms/order',{headers:csrfHeaders,data:{ids:[a,c,b]}})]);concurrentRoomOrderStatuses.push(...responses.map(response=>response.status()))}});
Then('every room reorder completes without a server error',async()=>{expect(concurrentRoomOrderStatuses).toHaveLength(16);expect(new Set(concurrentRoomOrderStatuses)).toEqual(new Set([200]))});
