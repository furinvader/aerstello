...
export function GuestPage() {
  ...
  const categories = products ? [...new Map(products.map((product)=>[product.categoryId,product.categoryName])).entries()] : [];
  ...
  return <main className="guest-shell">
    ...
    <div className="guest-tabs">
      <section><h2>{t('selfService')}</h2>{catalog.isError&&!products?<Card><Notice kind="error">{t('requestFailed')}</Notice><Button onClick={() => void catalog.refetch()}>{t('retry')}</Button></Card>:catalog.isPending?<Card><p className="muted">{t('loading')}</p></Card>:categories.length?categories.map(([categoryId,categoryName]) => <div key={categoryId} className="catalog-group"><h3>{localized(categoryName,language)}</h3><div className="product-grid">{products!.filter((product) => product.categoryId===categoryId).map((product) => <button className="product-tile" key={product.id} onClick={() => addProduct(product)} disabled={pendingAddProducts.has(product.id)}><span>{localized(product.name, language)}</span><strong>{formatMoney(product.priceCents, language)}</strong><Plus/></button>)}</div></div>):<Card><Empty>{t('empty')}</Empty></Card>}</section>
