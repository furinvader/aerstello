...
export interface OrderItem { id: string; productId: string; productName: LocalizedText; unitPriceCents: number; quantity: number; source: 'host' | 'guest'; status: 'provisional' | 'open'; provisionalUntil?: string; createdAt: string }
...
export interface Bill { id: string; number: number; venueName: string; guestName: string; roomName: string; totalCents: number; paymentMethod: string; settledAt: string; voidedAt?: string }
