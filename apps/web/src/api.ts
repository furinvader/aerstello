...
const localizedErrors: Partial<Record<string, Record<Language, string>>> = {
  ...
  ROOM_ORDER_CHANGED: { de: 'Die Zimmerreihenfolge wurde zwischenzeitlich geändert. Bitte die Liste neu laden.', it: 'L’ordine delle camere è stato modificato nel frattempo. Ricarica l’elenco.', en: 'The room order changed in the meantime. Reload the list.' },
  ...
  HOST_CHANGED: { de: 'Das Host-Konto wurde zwischenzeitlich geändert. Bitte die Liste neu laden.', it: 'L’account host è stato modificato nel frattempo. Ricarica l’elenco.', en: 'The host account changed in the meantime. Reload the list.' },
  ACCOUNT_CHANGED: { de: 'Das eigene Konto wurde zwischenzeitlich geändert. Bitte die Seite neu laden.', it: 'Il tuo account è stato modificato nel frattempo. Ricarica la pagina.', en: 'Your account changed in the meantime. Reload the page.' },
...
export async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (options.method && options.method !== 'GET') headers.set('x-aerstello-csrf', '1');
