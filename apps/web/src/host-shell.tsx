...
export function HostShell({children}:{children:ReactNode}) {
  ...
  useEffect(() => {
    ...
    ['access-request.changed','guest-access.changed','orders.changed','bills.changed','rooms.changed','guests.changed','catalog.changed','venue.changed','host-auth.changed'].forEach((event) => events.addEventListener(event, refresh));
    return () => events.close();
  }, [me.isSuccess, client]);
  ...
  const logout=async()=>{try{await api('/auth/logout',{method:'POST'})}catch{/* Local authenticated state must still be removed after an uncertain response. */}finally{client.clear();globalThis.location.assign('/login')}};
  const nav = [
