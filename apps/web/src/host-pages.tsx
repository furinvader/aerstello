...
import { formatMoney, localized, localizedTextSchema, orderBatchSchema, parseEuroCents, type Language, type LocalizedText } from '@aerstello/shared';
...
const uncertainOrderKey = (hostId: string) => `aerstello-uncertain-order:${hostId}`;

function isCurrentOrderRecovery(candidate: unknown, hostId: string): candidate is QueuedMutation {
  if (typeof candidate !== 'object' || candidate === null || Array.isArray(candidate)) return false;
  const mutation = candidate as QueuedMutation;
  const command = orderBatchSchema.safeParse(mutation.body);
  const display = mutation.display;
  if (!command.success || display?.kind !== 'order') return false;
  if (mutation.id !== command.data.mutationId
    || mutation.hostId !== hostId
    || command.data.originHostId !== hostId
    || mutation.path !== '/order-batches'
    || mutation.method !== 'POST'
    || typeof mutation.createdAt !== 'string'
    || display.guestId !== command.data.guestId
    || typeof display.guestName !== 'string'
    || typeof display.roomName !== 'string'
    || display.items.length !== command.data.items.length) return false;
  return display.items.every((item, index) => {
    const commandItem = command.data.items[index];
    return item.productId === commandItem?.productId
      && item.quantity === commandItem.quantity
      && Number.isSafeInteger(item.unitPriceCents)
      && item.unitPriceCents >= 0
      && localizedTextSchema.safeParse(item.productName).success;
  });
}

function loadUncertainOrder(hostId: string): QueuedMutation | null {
  try {
    const raw=localStorage.getItem(uncertainOrderKey(hostId));
    if (!raw) return null;
    const mutation: unknown = JSON.parse(raw);
    if (isCurrentOrderRecovery(mutation, hostId)) return mutation;
  } catch { /* Ignore and remove malformed recovery state below. */ }
  localStorage.removeItem(uncertainOrderKey(hostId));
  return null;
}
...
export function TakeOrdersPage() {
  ...
  const catalogReady=Boolean(products.data&&categories.data);
  ...
  const cartLines=stagedDisplay
    ? stagedDisplay.items.map((item)=>({product:{id:item.productId,name:item.productName,priceCents:item.unitPriceCents},quantity:item.quantity}))
  ...
  const add=(productId:string,change:number)=>{if(selectedGuest&&catalogReady&&!uncertainOrder&&!submitting)setCart(current=>updateOrderCart(current,productId,change))};
  const submit=async()=>{
    if(!guestId||(!stagedSubmission.current&&(!products.data||!categories.data||!selectedGuest||!cartLines.length)))return;
    setSubmitting(true);
...
function GuestDevicesModal({guest,onClose}:{guest:Guest;onClose:()=>void}){
  ...
  const devices=sessions.isPending
    ? <p className="muted">{t('loading')}</p>
    : sessions.isError
      ? <><Notice kind="error">{t('requestFailed')}</Notice><Button onClick={()=>void sessions.refetch()}>{t('retry')}</Button></>
      : sessions.data.data.length
        ? <div className="device-list">{sessions.data.data.map(session=><div key={session.id}><div className="device-icon"><Smartphone/></div><div className="grow"><strong>{session.userAgent.slice(0,55)}</strong><span>{t('accessExpires')} {new Date(session.expiresAt).toLocaleString(language)}</span></div><Button variant="ghost" onClick={()=>void revoke(session.id)}>{t('revoke')}</Button></div>)}</div>
        : <Empty>{t('empty')}</Empty>;
  return <Modal title={`${guest.name} · ${t('loggedInDevices')}`} onClose={onClose}>{devices}</Modal>;
}
...
export function RoomsPage(){
  ...
  if(!rooms.data)return <><PageHeader eyebrow={t('configuration')} title={t('rooms')}/><Card>{rooms.isError?<><Notice kind="error">{t('requestFailed')}</Notice><Button onClick={()=>void rooms.refetch()}>{t('retry')}</Button></>:<p className="muted">{t('loading')}</p>}</Card></>;
  return <>
...
const pendingDenialKey=(hostId:string)=>`aerstello-pending-denial:${hostId}`;
...
export function RequestsPage(){
  ...
  const queue=requests.isPending
    ? <Card><p className="muted">{t('loading')}</p></Card>
    : requests.isError
      ? <Card><Notice kind="error">{t('requestFailed')}</Notice><Button onClick={()=>void requests.refetch()}>{t('retry')}</Button></Card>
      : requests.data.data.length
        ? <div className="request-grid">{requests.data.data.map(request=><Card key={request.id} className="request-card"><div className="request-avatar"><UserRoundCheck/></div><div className="grow"><h2>{request.name}</h2><p><BedDouble/> {request.roomName}</p><span>{new Date(request.requestedAt).toLocaleTimeString(language)} · {request.language.toUpperCase()}</span></div><div className="request-actions"><Button disabled={!guests.data} onClick={()=>setApproving(request)}><Check/> {t('approve')}</Button><Button variant="ghost" disabled={Boolean(pendingDenial&&pendingDenial.requestId!==request.id)} onClick={()=>void denyRequest(request.id,request.name)}>{pendingDenial?.requestId===request.id?t('retry'):t('deny')}</Button></div></Card>)}</div>
        : !pendingDenial&&<Card><Empty>{t('empty')}</Empty></Card>;
  const guestDirectory=!guests.data&&(guests.isError
    ? <Card><Notice kind="error">{t('requestFailed')}</Notice><Button onClick={()=>void guests.refetch()}>{t('retry')}</Button></Card>
    : <Card><p className="muted">{t('loading')}</p></Card>);
  return <><PageHeader eyebrow={t('liveQueue')} title={t('requests')} actions={requests.data?.data.length?<span className="live-pill"><span/>{requests.data.data.length} {t('pending')}</span>:undefined}/>{pendingDenial&&<Card><Notice kind="error">{denialError||t('requestFailed')}</Notice><Button onClick={()=>void denyRequest(pendingDenial.requestId,pendingDenial.requestName)}>{t('retry')} · {pendingDenial.requestName}</Button></Card>}{denialError&&!pendingDenial&&<Notice kind="error">{denialError}</Notice>}{guestDirectory}{queue}{approving&&guests.data&&<ApproveRequestModal request={approving} guests={guests.data.data} onClose={()=>setApproving(undefined)} onApproved={async()=>{setApproving(undefined);await client.invalidateQueries()}}/>}</>;
}
...
export function SettingsPage(){
  ...
  if(host.role!=='admin')return <Card><Notice kind="error">{t('permissionRequired')}</Notice></Card>;
  if(venue.isPending)return <div className="splash">{t('loading')}</div>;
  if(venue.isError)return <Card><Notice kind="error">{t('requestFailed')}</Notice><Button onClick={()=>void venue.refetch()}>{t('retry')}</Button></Card>;
  const roomDirectory=!rooms.data
    ? rooms.isError
      ? <><Notice kind="error">{t('requestFailed')}</Notice><Button onClick={()=>void rooms.refetch()}>{t('retry')}</Button></>
      : <p className="muted">{t('loading')}</p>
    : rooms.data.data.length
      ? <div className="qr-grid">{rooms.data.data.map(room=><Card className="qr-card" key={room.id}><h3>{room.name}</h3><QRCodeSVG className="qr-code" value={`${base}/guest/request?room=${room.id}`} size={150} bgColor="#ffffff" fgColor="#07101b" level="M" marginSize={2}/><small>{t('roomPreselected')}</small></Card>)}</div>
      : <Empty>{t('empty')}</Empty>;
  return <><PageHeader eyebrow={`Aerstello · ${t('configuration')}`} title={t('settings')}/>{saved&&<Notice>{t('venueSaved')}</Notice>}{error&&<Notice kind="error">{error}</Notice>}<div className="two-column"><Card><h2>{t('venueIdentity')}</h2><p className="muted">{t('venueIdentityHelp')}</p><form className="stack" onSubmit={submit}><Field label={t('venueName')}><input value={name} onChange={e=>setName(e.target.value)} required maxLength={120} placeholder={t('venueName')}/></Field><Field label={t('defaultLanguage')}><select value={language} onChange={e=>setLanguage(e.target.value as Language)}><option value="de">Deutsch</option><option value="it">Italiano</option><option value="en">English</option></select></Field><Field label={t('timezone')}><input value={timezone} onChange={e=>setTimezone(e.target.value)} required/></Field><Button type="submit">{t('save')}</Button></form></Card><Card className="qr-card"><h2>{t('venueGuestAccess')}</h2><QRCodeSVG className="qr-code" value={`${base}/guest/request`} size={190} bgColor="#ffffff" fgColor="#07101b" level="M" marginSize={2}/><code>{base}/guest/request</code><Button variant="secondary" onClick={()=>window.print()} disabled={!rooms.data}><Printer/> {t('print')}</Button></Card></div><div className="section-heading"><h2>{t('roomQrCodes')}</h2></div>{roomDirectory}</>;
}
