...
Settlement submits the item count and total displayed in its confirmation, then locks the open tab and items and rejects any changed state. It creates the bill with an insertion-time database wall-clock timestamp and immutable bill lines, snapshots the current venue/guest/room/host identity, marks order items billed, and closes the tab in one transaction. Concurrent settlement and guest-item commands recheck their mutation keys after serialization so identical requests share one success. A later venue or host rename only affects operational UI and new bills. Dashboard daily sales classify each bill with its settlement-time venue timezone, so a later timezone edit cannot re-date historical revenue. Bills created before host-name snapshots existed are explicitly marked as having unavailable historical attribution rather than being backfilled from a mutable current profile. Admin reversal locks the guest, voids the bill through an audit event, and moves original order items into the guest's current tab, so reversal cannot race guest archival. Printed reversed bills retain a prominent void marker and reason.
