...
Billing, guest creation, access approval, catalog configuration, and venue settings are online-only. This boundary prevents duplicate settlement and unsafe configuration merges. Product and guest archival commands carry both the displayed entity version and a client mutation UUID, so stale requests are rejected and lost responses can be replayed without removing newer data. A room cannot be archived while it has active guests or pending access requests, and public request creation shares the room lock with archival.
