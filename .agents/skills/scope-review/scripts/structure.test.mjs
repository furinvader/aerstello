...
test('documentation links resolve and defines invocation and authority boundaries', () => {
  ...
  assert.match(contract, /assessment actually produced by change development,\s+or that exact development assessment preserved unchanged when imported from\s+its receipt-valid handoff, `taskPacketDigest` is the canonical terminal\s+`taskSetDigest`/u);
  ...
  assert.match(contract, /every PR-review-produced `integrated-head` assessment, whether its\s+authority is standalone or imported, `taskPacketDigest` instead retains the\s+exact immutable remediation packet identity for the classified root/u);
